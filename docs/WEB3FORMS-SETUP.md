...
which emails the enquiry to CTL's inbox. No Worker, no server, no build step
of ours in between.
...
`NEXT_PUBLIC_WEB3FORMS_KEY`, read by `ctl-roofing/lib/submitContact.ts`.
...
If CTL's volume is anywhere near it, that is a paid plan rather than a risk
to carry.
...
1. Go to <https://web3forms.com> and enter the address the enquiries should
   land in.
2. **Use CTL's own office inbox**, not Charlie's and not a personal account.
...
and a roofing contractor's form is filled in by people standing in a driveway
...
| | |
| --- | --- |
| The variable, and every other one | `ctl-roofing/.env.example` |
| The code, and why it is shaped this way | `ctl-roofing/lib/submitContact.ts` |
| The form itself | `ctl-roofing/components/Contact.tsx` |
